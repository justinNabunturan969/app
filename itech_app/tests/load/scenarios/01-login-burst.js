...
//
// Goal: prove the student-login RPC path can absorb 800 concurrent
// sign-in attempts without breaching the documented p95 / error-rate
// budgets, and that the per-IP rate limiter does NOT fire for a
// legitimate 800-strong class burst.
//
// What this exercises (against your real Supabase project):
//   1. 800 VUs each call POST /auth/v1/token?grant_type=password with a
//      unique student email + a known-bad password.
//   2. We measure p95 latency and the 429 / "Too many sign-in attempts"
//      rate. A *real* sign-in burst with 800 different identifiers
//      sharing one egress IP must not trip the per-IP lock — that's the
//      load test that proves the migration 0022 tuning is correct.
//
// Pass criteria:
//   - p95 < 1500ms under the burst
//   - < 1% of requests return 429 (lockout)
...
//          tests/load/scenarios/01-login-burst.js
//
// The student emails are generated as `loadtest+<vu>@<STUDENT_DOMAIN>`
// — they don't need to exist; the rate-limiter and timing-oracle
// behavior is what we care about.
...
export const options = {
  scenarios: {
    // The thesis claim: "800 students can log in at the same time"
    // (e.g. a class start at 7:30 AM). 800 VUs ramped over 30 seconds,
    // sustained for 60 seconds, then ramped down.
    login_burst: {
      executor: 'ramping-vus',
      startVUs: 0,
      stages: [
        { duration: '30s', target: 800 },  // ramp up: class fills the lab
        { duration: '60s', target: 800 },  // hold: students are still signing in
  ...
  thresholds: {
    ...
    'login_successes':   ['count>0'],       // at least some must succeed
  },
